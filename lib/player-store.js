...
const MANUAL_NON_ALL_STAR_KEYS = new Set([
  normalizeName('Zach Edey')
]);

function loadStarterAllStarAllowlist(listPath) {
  try {
...
class PlayerStore {
  constructor(options = {}) {
    ...
    this.startPlayerAllowlistPath = options.startPlayerAllowlistPath || path.join(process.cwd(), 'data', 'all-star-player-names.json');
    this.db = null;
    ...
    this.allStarKeys = [];
    this.startPlayerAllowlist = loadStarterAllStarAllowlist(this.startPlayerAllowlistPath);
  }
  ...
  loadIntoMemory() {
    ...
    this.allStarKeys = [...this.playersByKey.entries()]
      .filter(([key, player]) => {
        ...
        if (MANUAL_NON_ALL_STAR_KEYS.has(key)) return false;
        if (this.startPlayerAllowlist && !this.startPlayerAllowlist.has(normalizeLooseName(key))) return false;
