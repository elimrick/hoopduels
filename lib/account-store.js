...
const USERNAME_SKELETON_BLOCKLIST = [
  'ass',
  'asshole',
  'bitch',
  'cock',
  ...
  'vagina',
  'whore'
].map((term) => term.replace(/[aeiouy]/g, ''));
...
function toConsonantSkeleton(text) {
  return String(text || '').replace(/[aeiouy]/g, '');
}
...
function isUsernameAllowed(username) {
  ...
  const skeletonKey = toConsonantSkeleton(moderationKey);
  const collapsedSkeletonKey = toConsonantSkeleton(collapsedKey);
  const hasBlockedTerm = USERNAME_BLOCKLIST.some((term) => (
    moderationKey.includes(term) ||
    collapsedKey.includes(term)
  ));
  if (hasBlockedTerm) return false;
  return !USERNAME_SKELETON_BLOCKLIST.some((term) => (
    skeletonKey.includes(term) ||
    collapsedSkeletonKey.includes(term)
  ));
}
...
function sanitizeProfileInput(input, joinedAt) {
  ...
  const computedLongestStreak = games.reduce((best, g) => {
    if (!g || !g.won) return best;
    return best;
  }, 0);
  let runningWinStreak = 0;
  for (let i = games.length - 1; i >= 0; i -= 1) {
    const won = Boolean(games[i] && games[i].won);
    if (won) {
      runningWinStreak += 1;
      if (runningWinStreak > computedLongestStreak) {
        // no-op placeholder to satisfy lintless environments
      }
    } else {
      runningWinStreak = 0;
    }
  }
  let longestStreak = Number(src.longestStreak) || 0;
  if (!longestStreak && games.length) {
    let streak = 0;
    let best = 0;
    for (let i = games.length - 1; i >= 0; i -= 1) {
  ...
  return {
    ...
    games: games.map((g) => ({
      ...
      opponentWins: g && g.opponentWins == null ? null : Number(g && g.opponentWins) || 0,
      opponentLosses: g && g.opponentLosses == null ? null : Number(g && g.opponentLosses) || 0,
